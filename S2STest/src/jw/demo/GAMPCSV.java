...
public class GAMPCSV {
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i <= 200; i++) {
			options.addOption("cd" + i, "dimension" + i, true,
					"custom dimension index " + i);
		}
		...
		try {
			...
			if (line.hasOption("file")) {
				...
				throw new Exception("no value for argument -file");
			if (line.hasOption("cid")) {
				cid = line.getOptionValue("cid");
				System.out.println("cid=" + cid);
			} else
				throw new Exception("no value for argument -field");
			if (line.hasOption("tid")) {
				tid = line.getOptionValue("tid");
				System.out.println("tid=" + tid);
			} else
				throw new Exception("no value for argument -property");
			if (line.hasOption("ec")) {
				ec = line.getOptionValue("ec");
				System.out.println("ec=" + ec);
			} else
				throw new Exception("no value for argument -ec");
			if (line.hasOption("ea")) {
				ea = line.getOptionValue("ea");
				System.out.println("ea=" + ea);
			} else
				throw new Exception("no value for argument -ea");
			for (int i = 1; i <= 200; i++) {
		...
		CSVParser parser;
		URI uri;
		// user agent value to avoid being treated as a spam or bot
		...
		for (CSVRecord record : parser) {
			...
			if (dh != null)
				params.add(new BasicNameValuePair("dh", dh));
			if (dp != null)
				params.add(new BasicNameValuePair("dp", dp));
			try {
